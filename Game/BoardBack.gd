...
func _process(delta):
	...
	self.current_intensity = ease_kinda(self.current_intensity, self.target_intensity, delta, 0.05); 
...
func _physics_process(_delta):
	self.target_wavenumber = pow(2, max(0, -7 + $"../Blocks".tallest_column_height() + $"../Blocks".fractional_raise));
	self.color = $"../Blocks".pause * 1/4.0;

	if (self.board_options.grace_period != get_parent().grace):
		self.target_intensity = 0;
	elif $"../Blocks".pause > 0:
		self.target_intensity = 0.05;
	else:
